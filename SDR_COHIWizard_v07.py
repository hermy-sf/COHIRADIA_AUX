...
class statlst_gen_worker(QtCore.QThread):
    ...
    def stationsloop(self):
        with open(self.host.stations_filename, 'w', encoding='utf-8') as f:
    ...
    def autoscan_fun(self):
        ...
        for self.autoscan_ix in range(self.NUMSNAPS+1):
            ...
            if self.autoscan_ix > self.NUMSNAPS-1:
                ...
                data = self.host.readsegment()#TODO: replace by
                if len(data) == 10:
...
class WizardGUI(QMainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.ui.gridLayout_4.addWidget(self.canvas,4,0,1,5)
    ...
    def readsegment(self):
        ...
        if self.wavheader['nBitsPerSample'] == 16:
            ...
            invalid = np.empty(10, dtype=np.int16)
            invalid = np.linspace(0,9,10)
            size = self.fileHandle.readinto(dataraw)
            if size < self.DATABLOCKSIZE:
                return invalid
