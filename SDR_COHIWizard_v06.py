...
class autoscan_worker(QtCore.QThread):
    ...
    def autoscan_fun(self):
        ...
        for self.autoscan_ix in range(self.NUMSNAPS+1):
            ...
            self.host.progressvalue = self.position
            ...
            while self.__slots__[1] == False:
                time.sleep(0.01)
            self.host.horzscal = self.position
            if self.autoscan_ix > self.NUMSNAPS-1:
                self.autoscan_ix = 0  #??????????? necessary
            else:
                #print(f"autoindex:{self.autoscan_ix}")
                data = self.host.readsegment()#TODO: replace by slots
                pdata = self.host.ann_spectrum(self,data)#TODO: replace by slots
                ...
                datay = pdata["datay"]
                basel = pdata["databasel"] + self.host.Baselineoffset#TODO: replace by slots
        ...
        self.host.locs_union = self.locs_union #TODO: replace by slots
        self.host.freq_union = self.freq_union #TODO: replace by slots
        ...
        if os.path.isdir(self.host.my_dirname + '/' + self.host.my_filename) == False:
            os.mkdir(self.host.my_dirname + '/' + self.host.my_filename)#TODO: replace by slots

        #self.annotation_filename = self.my_dirname + '/' + self.my_filename + '/snrannotation.yaml'
        #TODO: check if file exists
        stream = open(self.host.annotation_filename, "w")#TODO: replace by slots
        yaml.dump(yamldata, stream)
        stream.close()
        self.host.annotation = self.annotation#TODO: replace by slots
...
class WizardGUI(QMainWindow):
    ...
    def Progressbarupdate(self):
        ...
        print(int(self.progressvalue/10))
        print('updating ####################### ')
        ...
        if os.path.exists(self.stations_filename) == False:
            ...
            with open(self.stations_filename, 'w', encoding='utf-8') as f:
                ...
                for ix in range(len(self.locs_union)):
                    ...
                    #self.statusupdateinst.set_0(int(progress))
            ...
            #self.statusupdatethread.quit()
            #self.statusupdatethread.deleteLater()
            status = {}
            status["freqindex"] = 0
    ...
    def interactive_station_select(self):
        ...
        ############TODO: getrennt schreiben in freq label und lineEdit 
    ...
    def ListClicked(self,item):
        ...
        if freq_ix < len(self.stations):
            ...
            self.ui.lineEdit.setText(country_string + ' | ' + programme_string)
            self.ui.labelFrequency.setText('f: ' + str(self.stations[freq_ix]['frequency']) + ' kHz')
            self.ui.lineEdit.setAlignment(QtCore.Qt.AlignLeft)
    ...
    def overwrite_header(self):
        ...
        if self.filetype == "dat":
            ...
            if usix == -1:
                ...
                msg.setInformativeText("dat file seems not to be a COHIRADIA RFCorder file, name convention not met, Abort renaming")
                msg.setWindowTitle("file not valid")
                msg.exec_()
                return False
            bbb = self.my_filename[0:usix-2]
            new_name = self.my_dirname + '/' + bbb + '_' + str(aaa) + '_' + str(int(np.round(self.wavheader["centerfreq"]/1000))) + 'kHz.wav'
            # Renaming the file
    ...
    def FileOpen(self):
        ...
        self.ui.label_Filename1.setText(self.my_filename + ext)
        self.ui.label_Filename2.setText(self.my_filename + ext)
        self.ui.label_Filename3.setText(self.my_filename + ext)
        self.stations_filename = self.my_dirname + '/' + self.my_filename + '/stations_list.yaml'
        self.status_filename = self.my_dirname + '/' + self.my_filename + '/status.yaml'
        self.annotation_filename = self.my_dirname + '/' + self.my_filename + '/snrannotation.yaml'
        ...
        if ext == ".dat": 
            ...
            if self.filetype == "dat" and self.filenameparams_extract() == False: # namecheck only if dat --> makes void all wav-related operation sin filenameextract
                ...
                if SDRtypestr.find('auxi') > -1:
                    self.sdrtype = 'AUXI'
                else:                
                    if SDRtypestr.find('rcvr') > -1:
    ...
    def fill_wavtable(self):
        ...
        if self.sdrtype == 'AUXI':
            ...
            msg = QMessageBox()
            msg.setIcon(QMessageBox.Critical)
            msg.setText("Header not valid")
            msg.setInformativeText("Perseus not yet supported, error in method fill_wavtable ")
            msg.setWindowTitle("Header not valid")
            msg.exec_()
